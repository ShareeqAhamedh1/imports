...
<div class="container toparea">
	<div class="underlined-title">
		<div class="editContent">
			<h1 class="text-center latestitems"><?= $_REQUEST['hscode'] ?></h1>
		</div>
	</div>

  <?php if(isset($_REQUEST['tot_cif'])){
    ?>
    <div class="container">
  ...
  <?php }
     ...
     ?>
		...
		<div class="row">
			...
			<div class="col-lg-6">
				<form class="" action="cal_results.php" method="post">
          ...
          <?php $sqlRange = "SELECT * FROM tbl_ex_ranges WHERE hs_sub_code='$hsid'";
                $rsRange = $conn->query($sqlRange);

                if ($rsRange->num_rows > 0) {
           ?>
          <div class="form-group">
            <label for="">Select Range</label>
            <select class="form-control" name="rang_id">
              <?php while ($rowRange = $rsRange->fetch_assoc()) {?>
                <option value="<?= $rowRange['range_id'] ?>"><?= $rowRange['r_from'] ?> to <?= $rowRange['r_to'] ?></option>
              <?php } ?>
            </select>
          </div>
          <div class="form-group">
            <label for="">Enter CC</label>
            <input type="text" class="form-control" name="cc" value="" placeholder="ex: 1000,1500,2000,7000.....">
          </div>
        <?php } ?>
          <div class="form-group">
